 ...
 * {@link sk.f4s.easytodev.tasks.domain.Useer}.
 * <p>
 ...
 */
public class UseerProperties {

	private UseerProperties() {
	}

	private static final UseerPropertiesImpl<Useer> sharedInstance = new UseerPropertiesImpl<Useer>(Useer.class);

	public static Property<Useer> id() {
		return sharedInstance.id();
	}

	public static Property<Useer> name() {
		return sharedInstance.name();
	}

	public static Property<Useer> role() {
		return sharedInstance.role();
	}

	public static Property<Useer> uuid() {
		return sharedInstance.uuid();
	}

	public static Property<Useer> createdDate() {
		return sharedInstance.createdDate();
	}

	public static Property<Useer> createdBy() {
		return sharedInstance.createdBy();
	}

	public static Property<Useer> lastUpdated() {
		return sharedInstance.lastUpdated();
	}

	public static Property<Useer> lastUpdatedBy() {
		return sharedInstance.lastUpdatedBy();
	}

	public static Property<Useer> version() {
		return sharedInstance.version();
	}

	/**
	 * This class is used for references to {@link sk.f4s.easytodev.tasks.domain.Useer}, i.e. nested property.
	 */
	public static class UseerProperty<T> extends UseerPropertiesImpl<T> implements Property<T> {
		private static final long serialVersionUID = 1L;

		public UseerProperty(String parentPath, String additionalPath, Class<T> owningClass) {
			super(parentPath, additionalPath, owningClass);
		}
	}

	protected static class UseerPropertiesImpl<T> extends PropertiesCollection {
		private static final long serialVersionUID = 1L;
		Class<T> owningClass;

		UseerPropertiesImpl(Class<T> owningClass) {
			super(null);
			this.owningClass = owningClass;
		}

		UseerPropertiesImpl(String parentPath, String additionalPath, Class<T> owningClass) {
		...
		public Property<T> role() {
			return new LeafProperty<T>(getParentPath(), "role", false, owningClass);
		}
